...
def get_directory_names(metadata_source):
    try:
        # Use ffprobe to get metadata for directory naming
...
def get_metadata_source(url):
    # Download mp3 file to fetch metadata to be embedded to the flac file
    command = [
        "spotdl",
        "--log-level",
        "CRITICAL",
    ...
    try:
        ...
        # Tracking the mp3 file to be used as metadata source for the current flac file being downloaded
        for temp_file in os.listdir('./temp_resources'):
            if temp_file.endswith('.mp3'):
                return temp_file
        
        return None # Safety measure in case there is no temp file returned
    except subprocess.CalledProcessError as e:
        print(f"{datetime.now().strftime("%H:%M:%S")}: An error occurred: {e}")
        return None

def extract_metadata(mp3_file):
    # Make the mp3 readable for metadata fetching
...
def embed_tag(mp3_file, flac_file):
    ...
    try:
        ...
        # Export the album image from the mp3 file
...
if __name__ == "__main__":
    ...
    flac_file = f'./Music/{artist.split('/')[0].strip()}/{album}/{title}.flac'  # Predefined location of the flac file
